module.exports = {
    siteUrl: `https://www.everythingtechnicalwriting.com`, // Site domain. Do not include a trailing slash!

    postsPerPage: 21, // Number of posts shown on paginated pages (changes this requires sometimes to delete the cache)

    siteTitleMeta: `Everything Technical Writing`, // This allows an alternative site title for meta data for pages.
    siteDescriptionMeta: `Your One Stop Blog For All Things Technical Writing in the Software Industry`,
    shareImageWidth: 1000, // Change to the width of your default share image
    shareImageHeight: 523, // Change to the height of your default share image

    shortTitle: `ETW`, // Used for App manifest e.g. Mobile Home Screen
    siteIcon: `favicon.png`, // Logo in /static dir used for SEO, RSS, and App manifest
    backgroundColor: `#e9e9e9`, // Used for Offline Manifest
    themeColor: `#15171A`, // Used for Offline Manifest
}
